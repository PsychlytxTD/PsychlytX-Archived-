...
generate_mean_widget<- function(input, output, session, panel_name, subscale_name, population_quantity, populations, input_population, sds, means, mean_sd_references, reliabilities, reliability_references,
...
mean_widget_reac<- reactive({
  ...
  panel_title<- h3(tags$strong(paste(panel_name)))

  mean_widget_list<-

    purrr::pmap(params_list_maker(subscale_name = subscale_name, population_quantity = population_quantity, populations = populations, input_population = input_population(),
                                  means = means, sds = sds, mean_sd_references = mean_sd_references, reliabilities = reliabilities, reliability_references = reliability_references,
                                  cutoffs = cutoffs, cutoff_names = cutoff_names,
                                  cutoff_references = cutoff_references, cutoff_quantity = cutoff_quantity)[c(1,3,5)],

                function(mean_sd_rel_ids, means, mean_sd_references) {

                  div(
                    column(width = 2,
                           numericInput(inputId = ns(mean_sd_rel_ids), label = "", value = means),
